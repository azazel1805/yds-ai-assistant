...
const callApi = async (action: string, payload: object): Promise<any> => {
  try {
    ...
    console.error(`callApi içinde hata oluştu (eylem: ${action}):`, error);
    if (error instanceof Error) {
...
export const analyzeQuestion = async (question: string): Promise<any> => {
  const result = await callApi('analyzeQuestion', { question });
...
export const generateQuestions = async (prompt: string, useJsonMode: boolean = false): Promise<any> => {
  const result = await callApi('generateQuestions', { prompt, useJsonMode });
  if (useJsonMode) {
    return parseJsonGracefully(result.text);
  }
  return result.text;
...
export const generateClozeTest = async (prompt: string): Promise<any> => {
  const result = await callApi('generateClozeTest', { prompt });
  return parseJsonGracefully(result.text);
};

export const generateListeningTask = async (difficulty: string): Promise<any> => {
    const result = await callApi('generateListeningTask', { difficulty });
    return parseJsonGracefully(result.text);
};

export const getReadingSummaryAndVocab = async (passage: string): Promise<any> => {
    const result = await callApi('getReadingSummaryAndVocab', { passage });
    return parseJsonGracefully(result.text);
};

export const getReadingQuestions = async (passage: string): Promise<any> => {
    const result = await callApi('getReadingQuestions', { passage });
    return parseJsonGracefully(result.text);
};

export const getPersonalizedFeedback = async (history: HistoryItem[]): Promise<any> => {
    const result = await callApi('getPersonalizedFeedback', { history });
    return parseJsonGracefully(result.text);
};

export const analyzeWrittenText = async (topic: string, text: string): Promise<any> => {
    const result = await callApi('analyzeWrittenText', { topic, text });
    return parseJsonGracefully(result.text);
};

// DÜZELTME: Bu fonksiyon artık doğru yerde, callApi'ye erişebiliyor.
...
export const getDictionaryEntry = async (word: string, language: string = 'Turkish'): Promise<string> => {
  const result = await callApi('getDictionaryEntry', { word, language });
  return result.text;
};

export const sendTutorMessage = async (history: ChatMessage[], message: string): Promise<string> => {
  const result = await callApi('sendTutorMessage', { history, message });
  return result.text;
};

export const getWritingTopic = async (): Promise<string> => {
    const result = await callApi('getWritingTopic', {});
    return result.text;
};
